...
{
    static const char * LUT[] = { "?", "float", "double" , "int16", "int32", "int64",
                                  "Vec3f", "Vec3d", "Mask", "Half", "uint32", "bool",
                                  "RGBA8", "Float4", "Float8", "Float16", "FloatN",
                                  "Vec4f", "Vec4d", "Index", "OnIndex", "End" };
...
__hostdev__ inline bool isValid(GridType gridType, GridClass gridClass)
{
    if (gridClass == GridClass::LevelSet || gridClass == GridClass::FogVolume) {
        ...
        return gridType == GridType::Index || gridType == GridType::OnIndex;
...
__hostdev__ inline GridType mapToGridType()
{
    ...
    } else if (is_same<BuildT, ValueOnIndex>::value) {
...
public:
    ...
    /// @brief Return a const pointer to the words if the bit mask
    template<typename WordT>
    __hostdev__ const WordT* getWords() const
    {
        return reinterpret_cast<const WordT*>(&mWords);
    }
...
public:
    ...
    __hostdev__ typename enable_if<is_same<T, ValueIndex>::value || is_same<T, ValueOnIndex>::value, uint64_t>::type
...
struct NANOVDB_ALIGN(NANOVDB_DATA_ALIGNMENT) LeafData<ValueIndex, CoordT, MaskT, LOG2DIM>
{
    ...
    MaskT<LOG2DIM> mValueMask; // LOG2DIM(3): 64B.
    uint64_t       mOffset, mPadding;// 8B offset to first value in this leaf node
    ...
    __hostdev__ uint32_t valueCount() const { return uint32_t(512); }// 8^3 = 2^9
    ...
    __hostdev__ uint64_t lastOffset() const {return mOffset + 511u;}// 2^9 - 1

    // if stats are available, they are always placed after the last voxel value in this leaf node
    __hostdev__ uint64_t getMin() const { return this->hasStats() ? mOffset + 512u : 0u; }
    __hostdev__ uint64_t getMax() const { return this->hasStats() ? mOffset + 513u : 0u; }
    __hostdev__ uint64_t getAvg() const { return this->hasStats() ? mOffset + 514u : 0u; }
    __hostdev__ uint64_t getDev() const { return this->hasStats() ? mOffset + 515u : 0u; }

    __hostdev__ uint64_t getValue(uint32_t i) const {return mOffset + i;}// dense leaf node with active and inactive voxels
...
// Partial template specialization of LeafData with ValueOnIndex
template<typename CoordT, template<uint32_t> class MaskT, uint32_t LOG2DIM>
struct NANOVDB_ALIGN(NANOVDB_DATA_ALIGNMENT) LeafData<ValueOnIndex, CoordT, MaskT, LOG2DIM>
{
    ...
    union {uint8_t mCountOn[8]; uint64_t mPadding;};// prefetch sum of active values per 64 bit words
    // No padding since by design it is 32B aligned
    ...
    {
        return mCountOn[6] + ((uint32_t(mCountOn[7] >> 6) & uint32_t(1)) << 8) + CountOn(mValueMask.template getWord<uint64_t>(7));
    }
    ...
    // return the offset to the last value indexed by this leaf node (disregarding optional stats)
    __hostdev__ uint64_t lastOffset() const {return mOffset + this->valueCount();}
...
#else
        uint32_t n = i >> 6;
        const uint64_t w = mValueMask.template getWord<uint64_t>(n), mask = uint64_t(1) << (i & 63u);
        if (!(w & mask)) return uint64_t(0);// if i'th value is inactive return offset to background value
        uint64_t sum = mOffset + CountOn(w & (mask - uint64_t(1)) );
        if (n--) sum += mCountOn[n] + ((uint32_t(mCountOn[7] >> n) & uint32_t(1)) << 8);
        return sum;
#endif
    }
...
{
    static_assert(is_same<IndexT, ValueIndex>::value || is_same<IndexT, ValueOnIndex>::value, "Expected ValueIndex or ValueOnIndex");
...
public:
    ...
    __hostdev__ ChannelAccessor(const NanoGrid<IndexT>& grid, uint32_t channelID = 0u)
        ...
        , mChannel(nullptr)
    {
        NANOVDB_ASSERT(grid.gridType()  == GridType::Index || grid.gridType() == GridType::OnIndex);
